...
CREATE OR REPLACE FUNCTION progress_queue(
  p_member_id UUID,
  p_citizen_id UUID,
  p_heartbeat_stale_seconds INT DEFAULT 180  -- 3 minutes
) RETURNS TABLE (
  action TEXT,
  turn_id UUID,
...
BEGIN
    ...
    IF v_next_entry IS NOT NULL THEN
      ...
      IF v_next_entry.citizen_id = p_citizen_id THEN
        RETURN QUERY SELECT
          ...
          15000;  -- Next heartbeat in 15s (you have the turn)
        RETURN;
      END IF;
    END IF;
  END IF;
  ...
  RETURN QUERY SELECT
    ...
    v_queue_len,
    CASE
      WHEN v_position = 0 THEN 15000    -- 15s - next up
